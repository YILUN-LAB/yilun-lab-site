...
export function FeaturedHighlights({ projects }: FeaturedHighlightsProps) {
  const { lead, supporting } = selectHighlights(projects);

  return (
    <section
      data-screen-label="Selected"
      id="selected"
      className="relative w-full bg-black/85 px-8 py-28 md:px-16 lg:px-20"
    >
      <motion.div {...fadeBlurIn(0)} className="mb-14">
        <div className="mb-6 font-body text-sm text-white/80">// Selected</div>
        ...
        <p className="mt-6 max-w-xl font-body text-base font-light leading-snug text-white/80 md:text-lg">
          A short list of works that feel most like the lab right now — performance, installation,
          sensory environment.
        </p>
      </motion.div>
...
function FeatureCard({ project }: { project: HighlightInput }) {
  return (
    <motion.a
      {...fadeBlurIn(0)}
    ...
    >
      <div className="liquid-glass relative aspect-[16/9] w-full overflow-hidden rounded-[1.25rem]">
        <img
          src={project.cover}
...
function SupportingCard({ project, delay }: { project: HighlightInput; delay: number }) {
  return (
    <motion.a
      {...fadeBlurIn(delay)}
      href={`/projects/${project.slug}`}
      className="group block"
    >
      <div className="liquid-glass relative aspect-[4/3] w-full overflow-hidden rounded-[1.25rem]">
        <img
          src={project.cover}
          alt={`${project.title} — cover`}
          loading="lazy"
